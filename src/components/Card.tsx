...
const Card = ({ data, isLoading }: Props) => {
  ...
  const saveComponentAsImage = () => {
    htmlToImage
      .toJpeg(document.getElementById('user-card') as HTMLElement, { quality: 0.95 })
  ...
  return (
    <div
      className='flex-col flex p-5 bg-gitBlack border justify-between border-gitGray rounded-lg h-[350px]'
      id='user-card'
    >
      {isLoading
        ? (
          ...
          <>
            <div className='flex flex-row space-x-5 items-center w-full'>
              ...
              <div className='justify-center item-center space-y-2 w-full'>
                <div className='flex flex-row items-center space-x-3'>
                  ...
                  <span title='GitHub'>
                    <a
                      className='font-mono text-base text-gitBlue hover:underline'
            ...
            <div className='flex flex-row justify-between items-center mt-5'>
              <div className='justify-start space-y-1'>
                ...
                <p className='font-mono font-bold text-lg bg-gitGray rounded-full text-center'>
                  {repos}
                </p>
              </div>
              <div className='justify-start space-y-1'>
            ...
            <div className='flex justify-end items-center space-x-5'>
              <span title='save card as image'>
                <button
                  className='text-gitGreen border hover:bg-gitGreen hover:text-white border-gitGreen py-1 px-2 rounded-full font-bold font-mono text-xs'
                  onClick={saveComponentAsImage}
                >
                  Save as image
                </button>
              </span>
